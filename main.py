...
Features: NL Intent Detection, Rate Limiting, Conversation Memory,
          Reply-Context Awareness, Deep Intelligence (historical trend +
          holder concentration), Portfolio Tracker, Price Alerts,
          Whitepaper Forensics, AMA/URL Intelligence
...
async def route_intent(
    ...
    # raw_text is the un-annotated user message (no reply-context prefix),
    # used for CA/URL/price extraction so regex patterns aren't confused
    # by injected reply-context text. Falls back to `text` if not passed
    # (e.g. from the voice handler, which has no reply-context concept).
    extraction_text = raw_text if raw_text is not None else text

    # ── CRYPTO AUDIT ──
    if intent == "crypto_audit":
        ca = extract_ca(extraction_text)
        try:
    ...
    # ── URL / AMA AUDIT ──
        ...
        url = extract_url(extraction_text)
        try:
    ...
    # ── ALERT SET ──
        ...
        ca = extract_ca(extraction_text)
        price = extract_price(extraction_text)
        low = extraction_text.lower()
    ...
    # ── HELP ──
        ...
        await update.message.reply_text(
            ...
            "CRYPTO AUDIT\n"
            "Send any contract address (CA). I will pull live data from DexScreener, "
            "cross-reference historical trend and holder concentration data, and issue "
            "a forensic verdict.\n\n"
            "BUSINESS AUDIT\n"
            ...
            "Reply directly to any of my earlier messages and I will use that message as context.\n\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Detect intent and extract CA/URL from the RAW text — the reply-context
    # text injected below should never confuse the regex patterns.
    intent = detect_intent(user_input)
    logger.info(f"Intent: {intent}")

    # Build the text that goes to the LLM, with reply-context prepended if
    # the user tagged/replied to an earlier message.
    ...
    try:
        await route_intent(intent, user_input_for_llm, user_id, update, context, raw_text=user_input)
    except Exception as e:
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # No reply-context concept for voice notes — raw_text defaults to `text`.
