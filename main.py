...
def extract_ca(text: str) -> Optional[str]:
    """Extract a contract address from anywhere in the text."""
...
# ─────────────────────────────────────────────
# INTENT DETECTOR
# Pure regex/keyword — zero API calls.
# Returns an intent string that the router acts on.
# ─────────────────────────────────────────────
INTENT_PATTERNS = {
...
def detect_intent(text: str) -> str:
    ...
    Priority order matters — crypto and URL are checked before NL intents.
    """
    clean = text.strip()
    ...
    # Default — business idea, greeting, general question
    return "general"
...
# ─────────────────────────────────────────────
# INTENT ROUTER
# The brain that acts on detected intents.
# ─────────────────────────────────────────────
async def route_intent(
    ...
    # ── PORTFOLIO ADD ──
        ...
        if not ca:
            await update.message.reply_text(
                "I can see you want to add a token but I did not find a contract address in your message. "
                "Send the CA alongside your request."
            )
            return
        if ca in portfolio:
            await update.message.reply_text(f"That token is already in your portfolio.")
            return
        ...
        # Verify the token exists and get its name
        try:
    ...
    # ── ALERT SET ──
        ...
        if not ca:
            await update.message.reply_text(
                ...
                "Send the CA and the target price in the same message.\n\n"
        ...
        # Determine direction from natural language
        if any(word in low for word in ["above", "over", "hits", "reaches", "crosses", "passes", "goes to"]):
            direction = "above"
        elif any(word in low for word in ["below", "under", "drops", "falls", "dips", "goes below"]):
            direction = "below"
        else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Only show "Auditing" for operations that take time
...
def main():
    ...
    # No slash command handlers — everything routes through handle_text
